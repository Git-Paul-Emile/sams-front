...
export function NewCommandeModal({ onClose }: { onClose: () => void }) {
  ...
  const [nf, setNf] = useState({ clientId: "", montant: "", commercialId: "" });
  ...
  function handleSave() {
    ...
    createCommande.mutate(payload, { onSuccess: onClose });
  }
    ...
    <Modal title="Nouvelle commande" onClose={onClose}>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 flex justify-end gap-3 pt-2">
          ...
          <Btn onClick={handleSave} disabled={createCommande.isPending}>Enregistrer la commande</Btn>
        </div>
      </div>
    </Modal>
  );
